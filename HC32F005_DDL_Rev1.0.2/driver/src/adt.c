...
void Adt6CompACalllback(void)
{

}

void adt_timer_ms_init(en_adt_unit_t enAdt,uint16_t MS )			//enAdt timer456  
{
//    uint16_t u16Period;
    en_adt_compare_t enAdtCompare;
    uint16_t u16Compare;
...
//    enAdt = AdTIM4;
    ...
    Adt_SetCompareValue(enAdt, enAdtCompare, u16Compare);    //通用比较基准值寄存器A设置
		...
		if(enAdt==AdTIM4)
		{
			Adt_ConfigIrq(enAdt, AdtCMAIrq, TRUE, Adt4CompACalllback);
		}
		else if(enAdt==AdTIM5)
		{
			Adt_ConfigIrq(enAdt, AdtCMAIrq, TRUE, Adt5CompACalllback);
		}
		else if(enAdt==AdTIM6)
		{
			Adt_ConfigIrq(enAdt, AdtCMAIrq, TRUE, Adt6CompACalllback);
		}
...
void adt_timer_us_init(en_adt_unit_t enAdt,uint16_t us )			//enAdt timer456  
{
