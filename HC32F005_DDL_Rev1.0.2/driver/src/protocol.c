...
DP_DEF dp_data;
FLASH_LIGHT_DATA_DEF flash_light_data;
unsigned char change_mode;
unsigned char scene_len;
char sta_cha_flag = 1;
unsigned int irq_cnt = 0;
unsigned int num_cnt = 0;
int flash_dir = 0;

unsigned long my_byte_to_int(unsigned char a,unsigned char b,unsigned char c,unsigned char d);
void my_str_to_hex(unsigned char *str,unsigned char *hex,int len);
void my_hex_to_str(unsigned char *hex,unsigned char *str,int len);
void hsv2rgb(float h, float s, float v);
void get_light_data();
extern void set_flash_data();
...
{
	{WIFI_DPID_SWITCH_LED, DP_TYPE_BOOL},
  {WIFI_DPID_WORK_MODE, DP_TYPE_ENUM},
  {WIFI_DPID_BRIGHT_VALUE, DP_TYPE_VALUE},
  {WIFI_DPID_TEMP_VALUE, DP_TYPE_VALUE},
  {WIFI_DPID_COLOUR_DATA, DP_TYPE_STRING},
  {WIFI_DPID_SCENE_DATA, DP_TYPE_STRING},
  {BT_DPID_SWITCH_LED, DP_TYPE_BOOL},
  {BT_DPID_WORK_MODE, DP_TYPE_ENUM},
  {BT_DPID_BRIGHT_VALUE, DP_TYPE_VALUE},
  {BT_DPID_TEMP_VALUE, DP_TYPE_VALUE},
  {BT_DPID_COLOUR_DATA, DP_TYPE_STRING},
  {BT_DPID_SCENE_DATA, DP_TYPE_STRING},
};
...
//  #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
  //此代码为平台自动生成，请按照实际数据修改每个可下发可上报函数和只上报函数
	...
	my_hex_to_str(dp_data.SCENE_DATA,dp_data.SCENE_DATA_LOAD,scene_len);
	if(KEY_BT==0)
	{
		mcu_dp_bool_update(BT_DPID_SWITCH_LED,dp_data.SWITCH); //BOOL型数据上报;
		mcu_dp_enum_update(BT_DPID_WORK_MODE,dp_data.WORK_MODE); //枚举型数据上报;
		mcu_dp_value_update(BT_DPID_BRIGHT_VALUE,dp_data.BRIGHT); //VALUE型数据上报;
		mcu_dp_value_update(BT_DPID_TEMP_VALUE,dp_data.COL_TEMPERATURE); //VALUE型数据上报;
		mcu_dp_string_update(BT_DPID_COLOUR_DATA,dp_data.COLOUR_DATA_UPLAOD,12); //STRING型数据上报;
		mcu_dp_string_update(BT_DPID_SCENE_DATA,dp_data.SCENE_DATA_LOAD,scene_len); //STRING型数据上报;
	}
	else
	{
		mcu_dp_bool_update(WIFI_DPID_SWITCH_LED,dp_data.SWITCH); //BOOL型数据上报;
		mcu_dp_enum_update(WIFI_DPID_WORK_MODE,dp_data.WORK_MODE); //枚举型数据上报;
		mcu_dp_value_update(WIFI_DPID_BRIGHT_VALUE,dp_data.BRIGHT); //VALUE型数据上报;
		mcu_dp_value_update(WIFI_DPID_TEMP_VALUE,dp_data.COL_TEMPERATURE); //VALUE型数据上报;
		mcu_dp_string_update(WIFI_DPID_COLOUR_DATA,dp_data.COLOUR_DATA_UPLAOD,12); //STRING型数据上报;
		mcu_dp_string_update(WIFI_DPID_SCENE_DATA,dp_data.SCENE_DATA_LOAD,scene_len); //STRING型数据上报;	
	}
}
...
//			send_light_data(light_data.RED_VAL, light_data.GREEN_VAL, light_data.BLUE_VAL, light_data.WHITE_VAL, light_data.WARM_VAL);
  ...
  //处理完DP数据后应有反馈
	if(KEY_BT==0)
	{
		ret = mcu_dp_bool_update(BT_DPID_SWITCH_LED,switch_led);
	}
	else
	{
		ret = mcu_dp_bool_update(WIFI_DPID_SWITCH_LED,switch_led);
	}
  if(ret == SUCCESS)
    return SUCCESS;
  else
...
static unsigned char dp_download_work_mode_handle(const unsigned char value[], unsigned short length)
{
  ...
  //处理完DP数据后应有反馈
	if(KEY_BT==0)
	{
		ret = mcu_dp_enum_update(BT_DPID_WORK_MODE,work_mode);
	}
	else
	{
	  ret = mcu_dp_enum_update(WIFI_DPID_WORK_MODE,work_mode);
	}
...
static unsigned char dp_download_bright_value_handle(const unsigned char value[], unsigned short length)
{
  ...
  //处理完DP数据后应有反馈
	if(KEY_BT==0)
	{
		ret = mcu_dp_value_update(BT_DPID_BRIGHT_VALUE,bright_value);
	}
	else
	{
	  ret = mcu_dp_value_update(WIFI_DPID_BRIGHT_VALUE,bright_value);
	}
...
static unsigned char dp_download_temp_value_handle(const unsigned char value[], unsigned short length)
{
  ...
  temp_value = mcu_get_dp_download_value(value,length);
	...
	printf("temp_value=%d\r\n",temp_value);
#endif
	dp_data.COL_TEMPERATURE=temp_value;
	get_light_data();
	Timer2_init(6);
  ...
  //处理完DP数据后应有反馈
	if(KEY_BT==0)
	{
		ret = mcu_dp_value_update(BT_DPID_TEMP_VALUE,temp_value);
	}
	else
	{
	  ret = mcu_dp_value_update(WIFI_DPID_TEMP_VALUE,temp_value);
	}
...
static unsigned char dp_download_colour_data_handle(const unsigned char value[], unsigned short length)
{
  ...
  unsigned char string_data[12];
	...
	my_str_to_hex(string_data,dp_data.COLOUR_DATA,length);
	get_light_data();
	Timer2_init(6);
  ...
  //处理完DP数据后应有反馈
	...
	if(KEY_BT==0)
	{
		ret = mcu_dp_string_update(BT_DPID_COLOUR_DATA,value,length);
	}
	else
	{
	  ret = mcu_dp_string_update(WIFI_DPID_COLOUR_DATA,value,length);
	}
...
static unsigned char dp_download_scene_data_handle(const unsigned char value[], unsigned short length)
{
  ...
  my_str_to_hex(string_data,dp_data.SCENE_DATA,length);
//	set_flash_data();
	set_scene_data();
  ...
  //处理完DP数据后应有反馈
	if(KEY_BT==0)
	{
		ret = mcu_dp_string_update(BT_DPID_SCENE_DATA,value,length);
	}
	else
	{
	  ret = mcu_dp_string_update(WIFI_DPID_SCENE_DATA,value,length);
	}
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
  ...
  ***********************************/
  ...
  switch(dpid)
  {
    case WIFI_DPID_SWITCH_LED:
      //开关处理函数
      ret = dp_download_switch_led_handle(value,length);
      break;
    case WIFI_DPID_WORK_MODE:
      //模式处理函数
      ret = dp_download_work_mode_handle(value,length);
      break;
    case WIFI_DPID_BRIGHT_VALUE:
      //亮度值处理函数
      ret = dp_download_bright_value_handle(value,length);
      break;
    case WIFI_DPID_TEMP_VALUE:
      //冷暖值处理函数
      ret = dp_download_temp_value_handle(value,length);
      break;
    case WIFI_DPID_COLOUR_DATA:
      //彩光处理函数
      ret = dp_download_colour_data_handle(value,length);
      break;
    case WIFI_DPID_SCENE_DATA:
      //场景处理函数
      ret = dp_download_scene_data_handle(value,length);
      break;
		
		 case BT_DPID_SWITCH_LED:
      //开关处理函数
      ret = dp_download_switch_led_handle(value,length);
      break;
    case BT_DPID_WORK_MODE:
      //模式处理函数
      ret = dp_download_work_mode_handle(value,length);
      break;
    case BT_DPID_BRIGHT_VALUE:
      //亮度值处理函数
      ret = dp_download_bright_value_handle(value,length);
      break;
    case BT_DPID_TEMP_VALUE:
      //冷暖值处理函数
      ret = dp_download_temp_value_handle(value,length);
      break;
    case BT_DPID_COLOUR_DATA:
      //彩光处理函数
      ret = dp_download_colour_data_handle(value,length);
      break;
    case BT_DPID_SCENE_DATA:
      //场景处理函数
      ret = dp_download_scene_data_handle(value,length);
      break;
...
unsigned long my_byte_to_char(unsigned char a,unsigned char b)
{
...
void hsv2rgb(float h, float s, float v)
{
	...
	color_b = b;
#ifdef DEBUG_PRINTF
	printf("color_r=%d,color_g=%d,color_b=%d\r\n",color_r,color_g,color_b);
#endif
}
...
void get_light_data()
{
	...
	{
		case WHITE_MODE:
			light_data.WHITE_VAL = dp_data.BRIGHT*dp_data.COL_TEMPERATURE/1000;
			light_data.WARM_VAL = dp_data.BRIGHT - light_data.WHITE_VAL;
			light_data.RED_VAL = 0;
			light_data.GREEN_VAL =0;
			light_data.BLUE_VAL = 0;
#ifdef DEBUG_PRINTF
		printf("RED_VAL=%d,GREEN_VAL=%d,BLUE_VAL=%d,WHITE_VAL=%d,WARM_VAL=%d\r\n",light_data.RED_VAL,light_data.GREEN_VAL,light_data.BLUE_VAL,light_data.WHITE_VAL,light_data.WARM_VAL);
#endif
		break;
		
		case COLOUR_MODE:
			light_data.HUE=my_byte_to_int(dp_data.COLOUR_DATA[0], dp_data.COLOUR_DATA[1],dp_data.COLOUR_DATA[2], dp_data.COLOUR_DATA[3]);
			light_data.SATURATION=my_byte_to_int(dp_data.COLOUR_DATA[4], dp_data.COLOUR_DATA[5],dp_data.COLOUR_DATA[6], dp_data.COLOUR_DATA[7]);
			light_data.VALUE=my_byte_to_int(dp_data.COLOUR_DATA[8], dp_data.COLOUR_DATA[9],dp_data.COLOUR_DATA[10], dp_data.COLOUR_DATA[11]);
			hsv2rgb(light_data.HUE,light_data.SATURATION,light_data.VALUE);
			light_data.RED_VAL=color_r;
			light_data.GREEN_VAL=color_g;
			light_data.BLUE_VAL=color_b;
			light_data.WHITE_VAL = 0;
			light_data.WARM_VAL = 0;
#ifdef DEBUG_PRINTF
		printf("RED_VAL=%d,GREEN_VAL=%d,BLUE_VAL=%d,WHITE_VAL=%d,WARM_VAL=%d\r\n",light_data.RED_VAL,light_data.GREEN_VAL,light_data.BLUE_VAL,light_data.WHITE_VAL,light_data.WARM_VAL);
			...
			case SCENE_MODE:
				change_mode=my_byte_to_char(dp_data.SCENE_DATA[6],dp_data.SCENE_DATA[7]);
				if(change_mode==statical)
				{
					light_data.HUE=my_byte_to_int(dp_data.SCENE_DATA[8],dp_data.SCENE_DATA[9],dp_data.SCENE_DATA[10],dp_data.SCENE_DATA[11]);
					light_data.SATURATION=my_byte_to_int(dp_data.SCENE_DATA[12],dp_data.SCENE_DATA[13],dp_data.SCENE_DATA[14],dp_data.SCENE_DATA[15]);
					light_data.VALUE=my_byte_to_int(dp_data.SCENE_DATA[16],dp_data.SCENE_DATA[17],dp_data.SCENE_DATA[18],dp_data.SCENE_DATA[19]);
					hsv2rgb(light_data.HUE,light_data.SATURATION,light_data.VALUE);
					light_data.RED_VAL =color_r;
					light_data.GREEN_VAL =color_g;
					light_data.BLUE_VAL = color_b;
					flash_light_data.BRIGHT =my_byte_to_int(dp_data.SCENE_DATA[20],dp_data.SCENE_DATA[21],dp_data.SCENE_DATA[22],dp_data.SCENE_DATA[23]);
					flash_light_data.COL_TEMP = my_byte_to_int(dp_data.SCENE_DATA[24],dp_data.SCENE_DATA[25],dp_data.SCENE_DATA[26],dp_data.SCENE_DATA[27]);
					light_data.WHITE_VAL=flash_light_data.BRIGHT*flash_light_data.COL_TEMP/1000;
					light_data.WARM_VAL = flash_light_data.BRIGHT - light_data.WHITE_VAL;
				}
				else
				{
					light_data.WHITE_VAL = 0;
					...
					flash_light_data.BRIGHT = my_byte_to_int(dp_data.SCENE_DATA[20], dp_data.SCENE_DATA[21],dp_data.SCENE_DATA[22],dp_data.SCENE_DATA[23]);
					flash_light_data.COL_TEMP = my_byte_to_int(dp_data.SCENE_DATA[24], dp_data.SCENE_DATA[25],dp_data.SCENE_DATA[26],dp_data.SCENE_DATA[27]);
					flash_light_data.SPEED = 140 - (my_byte_to_char(dp_data.SCENE_DATA[2], dp_data.SCENE_DATA[3]));
					flash_light_data.NUM =(scene_len-2)/26;
					for(i=0; i<flash_light_data.NUM; i++)
					{
						light_data.HUE=my_byte_to_int((dp_data.SCENE_DATA[i*26+8]), (dp_data.SCENE_DATA[i*26+9]),(dp_data.SCENE_DATA[i*26+10]),(dp_data.SCENE_DATA[i*26+11]));
						light_data.SATURATION=my_byte_to_int((dp_data.SCENE_DATA[i*26+12]), (dp_data.SCENE_DATA[i*26+13]),(dp_data.SCENE_DATA[i*26+14]),(dp_data.SCENE_DATA[i*26+15]));
						light_data.VALUE=my_byte_to_int((dp_data.SCENE_DATA[i*26+16]), (dp_data.SCENE_DATA[i*26+17]),(dp_data.SCENE_DATA[i*26+18]),(dp_data.SCENE_DATA[i*26+19]));
						flash_light_data.BRIGHT =my_byte_to_int(dp_data.SCENE_DATA[i*26+20],dp_data.SCENE_DATA[i*26+21],dp_data.SCENE_DATA[i*26+22],dp_data.SCENE_DATA[i*26+23]);
						flash_light_data.COL_TEMP = my_byte_to_int(dp_data.SCENE_DATA[i*26+24],dp_data.SCENE_DATA[i*26+25],dp_data.SCENE_DATA[i*26+26],dp_data.SCENE_DATA[i*26+27]);
						hsv2rgb(light_data.HUE,light_data.SATURATION,light_data.VALUE);
						...
						flash_light_data.data_group[i].WHITE_VAL=flash_light_data.BRIGHT*flash_light_data.COL_TEMP/1000;
						flash_light_data.data_group[i].WARM_VAL=flash_light_data.BRIGHT - flash_light_data.data_group[i].WHITE_VAL;
						color[i]=light_data.HUE;
					}
