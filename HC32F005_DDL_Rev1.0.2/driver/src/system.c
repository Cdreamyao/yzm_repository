...
extern void set_default_value();
extern void get_light_data();

uint8_t bt_wifi_mode=0;
...
static void product_info_update(void)
{
  unsigned char length = 0;
	...
	if(KEY_BT==0)
	{
	  length = set_wifi_uart_buffer(length,(unsigned char *)BT_PRODUCT_KEY,my_strlen((unsigned char *)BT_PRODUCT_KEY));
	...
	else
	{
	  length = set_wifi_uart_buffer(length, "{\"p\":\"", my_strlen("{\"p\":\""));
		length = set_wifi_uart_buffer(length,(unsigned char *)WIFI_PRODUCT_KEY,my_strlen((unsigned char *)WIFI_PRODUCT_KEY));
...
void data_handle(unsigned short offset)
{
  ...
  case WIFI_STATE_CMD:                                  //wifi工作状态	
    ...
    wifi_uart_write_frame(WIFI_STATE_CMD,0);
		work_state=mcu_get_wifi_work_state();
		if(work_state==SMART_CONFIG_STATE)
		{
			set_default_value();
			adt_timer_ms_init(AdTIM5,250);
		}
		else if( work_state == AP_STATE && KEY_BT == 1 )
		{
				set_default_value();
