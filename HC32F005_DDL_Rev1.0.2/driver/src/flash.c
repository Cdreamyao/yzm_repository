...
extern uint32_t 		 scene_len_Addr;
extern uint32_t			 SCENE_DATA_Addr;
extern unsigned char scene_len;


en_result_t set_scene_data()
{
		int i;
    en_result_t             enResult = Ok;    
	
		Flash_SectorErase(SCENE_DATA_Addr);
	
    if (FLASH_END_ADDR < SCENE_DATA_Addr)
    {
        enResult = ErrorInvalidParameter;
        return (enResult);
    }
    ...
    //write data
    *((volatile uint8_t*)scene_len_Addr) = scene_len;
    for(i=0;i<scene_len;i++)
		{
			*((volatile uint8_t*)SCENE_DATA_Addr+i*2) = dp_data.SCENE_DATA[i];
		}
    //busy?
    while (TRUE == M0P_FLASH->CR_f.BUSY)
    {
