...
#include "pca.h"
#include "wifi.h"
//uint8_t          u8CcaplData = 255;
//uint8_t          u8CcaphData = 255-255;
...
//    Clk_SetPeripheralGate(ClkPeripheralGpio, TRUE);
    ...
    Gpio_SetFunc_PCA_CH0_P25(0);
		Gpio_SetFunc_PCA_CH1_P24(0);
		Gpio_SetFunc_PCA_CH2_P26(0);
		Gpio_SetFunc_PCA_CH3_P03(0);
		Gpio_SetFunc_PCA_CH4_P36(0);
		...
		if (Ok != Pca_CapModConfig(Module2, &stcModConfig))
    {
		...
		if (Ok != Pca_CapModConfig(Module3, &stcModConfig))
    {
        enResult = Error;
    }
...
//		
//		Pca_CapDataLSet(Module1, u8CcaplData);
//    Pca_CapDataHSet(Module1, u8CcaphData);
//		
//		Pca_CapDataLSet(Module2, u8CcaplData);
//    Pca_CapDataHSet(Module2, u8CcaphData);
//		
//		Pca_CapDataLSet(Module3, u8CcaplData);
//    Pca_CapDataHSet(Module3, u8CcaphData);
//		
...
void send_light_data(uint16_t R,uint16_t G,uint16_t B,uint16_t C,uint16_t W)
{
	R/=3.92;G/=3.92;B/=3.92;C/=3.92;W/=3.92;
	if(R==0)
	{
		Pca_CapDataLSet(Module3, 255);
	}
	else
	{
		Pca_CapDataHSet(Module3, 255-R);
	}
	if(G==0)
	{
		Pca_CapDataLSet(Module1, 255);
	}
	else
	{
		Pca_CapDataHSet(Module1, 255-G);
	}
	if(B==0)
	{
		Pca_CapDataLSet(Module0, 255);	
	}	
	else
	{
		Pca_CapDataHSet(Module0, 255-B);
	}
	if(C==0)
	{
		Pca_CapDataLSet(Module2, 255);
	}
	else
	{
		Pca_CapDataHSet(Module2, 255-C);
	}
	if(W==0)
	{
		Pca_CapDataLSet(Module4, 255);
	}
	else
	{
		Pca_CapDataHSet(Module4, 255-W);
	}	
	
#ifdef DEBUG_PRINTF
	printf("R=%d,G=%d,B=%d,C=%d,W=%d\r\n",R,G,B,C,W);
#endif
}
